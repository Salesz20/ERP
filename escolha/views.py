...
def contas(request):
    ...
    empresas = Company.objects.filter(name__icontains=query).order_by('name') if query else Company.objects.all().order_by('name')
...
def contatos(request):
    query = request.GET.get('q', '')
    contatos_qs = Contact.objects.filter(name__icontains=query).order_by('name') if query else Contact.objects.all().order_by('name')
    ...
    if request.method == 'POST':
        ...
        if nome and empresa_nome:
            try:
                empresa = Company.objects.get(name=empresa_nome)
            except Company.DoesNotExist:
                erro = f"A empresa '{empresa_nome}' não existe. Cadastre-a em Contas primeiro."
            else:
                nome_completo = f"{nome} {sobrenome}".strip()
                if Contact.objects.filter(name__iexact=nome_completo, company=empresa).exists():
                    erro = "Esse contato já existe nessa empresa!"
                else:
                    Contact.objects.create(company=empresa, name=nome_completo, email=email or None, role=cargo or None)
...
# ✅ AGORA ESSA VIEW REDIRECIONA PRO KANBAN AUTOMATICAMENTE
@login_required
...
@login_required
def tarefas(request):
    tarefas = Task.objects.all().order_by('-due_date')
...
def detalhes_empresa(request, empresa_id):
    ...
    if empresa_id == 0:
        if request.method == 'POST':
            ...
            oportunidades = request.POST.get('oportunidades', '').strip()
            ...
            if not nome:
            ...
            elif Company.objects.filter(name__iexact=nome).exists():
                ...
                for arquivo in arquivos:
                    CompanyAttachment.objects.create(company=empresa, arquivo=arquivo)
        ...
        empresa = get_object_or_404(Company, id=empresa_id)
...
def lista_contatos(request):
    ...
    contatos_list = Contact.objects.all().select_related('company').order_by('name')

    if query:
        contatos_list = contatos_list.filter(name__icontains=query)

    paginator = Paginator(contatos_list, 50)  # 👈 50 CONTATOS POR PÁGINA
...
def detalhes_oportunidade(request, contato_id=None, oportunidade_id=None):
    ...
    if oportunidade_id:
        oportunidade = get_object_or_404(Opportunity, id=oportunidade_id)
    elif contato:
        oportunidade = Opportunity.objects.filter(contato=contato).last()
        if not oportunidade:
    ...
    if request.method == 'POST' and oportunidade:
        ...
        if not erro:
            ...
            # ✅ AGORA ELE SÓ RECARREGA A MESMA PÁGINA (NÃO VAI PRO KANBAN)
            return redirect('escolha:detalhes_oportunidade', contato_id=contato.id, oportunidade_id=oportunidade.id)
...
# -----------------------------
# View para mostrar todas oportunidades (Kanban)
# -----------------------------
@login_required
def oportunidades_view(request):
    ...
    for stage in stages:
        oportunidades_por_stage[stage] = Opportunity.objects.filter(stage__iexact=stage).order_by('-id')
    return render(request, 'crm/crm_opcoes/oportunidades.html', {
        'oportunidades': oportunidades_por_stage,
        'stages': stages,
    })

# Quando você arrasta o card no Kanban, e atualizar a oportunidade no banco de dados, sem precisar recarregar a página

# -----------------------------
# Atualiza o stage da oportunidade via AJAX (kanban)
# -----------------------------
@csrf_exempt
@login_required
def update_oportunidade_stage(request):
    if request.method == "POST":
        try:
            ...
            oportunidade = Opportunity.objects.get(id=opp_id)
...
# ------------------------------
# MARCAR TAREFA COMO CONCLUÍDA (opcional)
# ------------------------------
...
def popup_tarefas(request, pk):
    oportunidade = get_object_or_404(Opportunity, id=pk)
    hoje = timezone.now().date()

    tarefas = Task.objects.filter(opportunity=oportunidade)

    # LÓGICA AUTOMÁTICA
    tarefas_concluidas = tarefas.filter(done=True)

    tarefas_atrasadas = tarefas.filter(done=False, due_date__lt=hoje)

    tarefas_execucao = tarefas.filter(done=False, due_date__gte=hoje)
...
# Atualizar tarefa (editar descrição / due_date)
def update_task(request):
    try:
        ...
        if due_date_raw:
            # espera YYYY-MM-DD
            try:
                dd = datetime.datetime.strptime(due_date_raw, "%Y-%m-%d").date()
                task.due_date = dd
            except Exception as e:
...
# body: { id: <int>, column: 'atrasadas'|'execucao'|'concluidas' }
def move_task(request):
    try:
        ...
        if column == 'concluidas':
            ...
            # garante due_date < hoje
            if task.due_date >= hoje:
                task.due_date = hoje - datetime.timedelta(days=1)
        else:
...
def lista_tarefas(request):
    ...
    tarefas = Task.objects.select_related('opportunity').all().order_by('due_date')

    # Separa por status
    tarefas_concluidas = [t for t in tarefas if t.done]
    tarefas_execucao = [t for t in tarefas if not t.done and t.due_date >= hoje]
    tarefas_atrasadas = [t for t in tarefas if not t.done and t.due_date < hoje]
